<?php
	...
	$listaProveedoresCerdo = listaProveedoresCerdo();
...
<head>
	...
	<style>
        ...
        #jtable th:nth-child(2), #jtable td:nth-child(2) {
            width: 10%; /* Ancho de la quinta columna */
        }
        #jtable th:nth-child(3), #jtable td:nth-child(3) {
            width: 27%; /* Ancho de la quinta columna */
        }
...
<body>
						...
						<div class="card-body">
							...
							<div class="row">
								<div class="col-md-12 d-flex justify-content-between">
									<button class="btn btn-primary" style="visibility: hidden;">Excel</button>
									<button class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#modalNuevoProveedor" title="Agregar Nueva Guia" id="crearProveedor">AGREGAR GUIA DE RECEPCIÓN</button>
									<button class="btn btn-primary" id="exportarExcel">Excel</button>
								</div>
								<div class="col-md-12 d-flex justify-content-center">
									<h5 style="color: red;display: none" class="mt-1" id="alertItem">La ultima guia no tiene items</h5>
								</div>
								<div class="col-md-12">
				...
				<div class="modal-body">
					<form method="POST" id="criteriosForm">
						<div class="row">
							...
							<div class="col-md-6">
								<div class="form-group label-floating" id="divProveedor" style="display: none;">
									Proveedor Cerdo<span class="text-danger">*</span>
									<select name="proveedorCerdo" id="proveedorCerdo" class="form-control">
										<option value="">Seleccione un proveedor</option>
										<?php foreach ($listaProveedoresCerdo as $proveedor): ?>
											<option value="<?= $proveedor[0] ?>"><?= $proveedor[1]; ?></option>
										<?php endforeach ?>
									</select>
								</div>
								<div class="alert alert-danger" role="alert" id="proveedorCerdoE" style="display: none">
									Debes seleccionar un proveedor de cerdo
								</div>
							</div>
							...
							<div class="col-11 col-md-5">
								...
								<div class="form-group label-floating" id="conseGuiaDiv"  style="width: 110%;">
							...
							<div class="col-11 col-md-5">
								...
								<div class="form-group label-floating" id="conseGuiaDiv">
									<select class="form-control" id="placa" name="placa" style="width: 110%;">
					...
					<form method="POST" id="criteriosForm">
						<div class="row">
							<div class="col-12 col-md-6">
								Cedula
								<div class="form-group label-floating" id="codigoAutoDiv">
									<input type="number" class="form-control" autocomplete="off" id="cedula" name="cedula" placeholder="Ingrese Cedula">
									<input type="hidden" id="id_guia">
	...
	<div class="modal fade" id="modalCriterios" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" style="background-color: #00000042 !important; z-index:1051">
	...
	<div class="modal fade" id="modalExportarExcel" data-bs-backdrop="static" data-bs-keyboard="true" tabindex="-1" aria-labelledby="modalExportarExcelLabel" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				<div class="modal-header text-center">
					<button type="button" class="btn-close" style="visibility: hidden;"></button>
					<h1 class="modal-title fs-5" id="modalExportarExcelLabel">Exportar Excel</h1>
					<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
				</div>
				<div class="modal-body">
					<div class="row">
						<div class="col-md-12 mb-2">
							<label for="proveedorExcel" class="form-label fw-bold">Proveedor</label>
							<select name="proveedorExcel" id="proveedorExcel" class="form-control">
								<option value="">Seleccione un proveedor</option>
								<?php foreach ($listaProveedoresCerdo as $proveedor) : ?>
									<option value="<?= $proveedor[0]; ?>"><?= $proveedor[1]; ?></option>
								<?php endforeach; ?>
							</select>
							<div class="alert alert-danger" role="alert" id="proveedorExcelE" style="display: none">
								<i class="bi bi-exclamation-triangle"></i> Debe seleccionar un proveedor
							</div>
						</div>

						<div class="col-md-6 mb-2">
							<label for="fechaInicioExcel" class="form-label fw-bold">Fecha Inicio</label>
							<input type="date" name="fechaInicioExcel" id="fechaInicioExcel" class="form-control">
							<div class="alert alert-danger" role="alert" id="fechaInicioExcelE" style="display: none">
								<i class="bi bi-exclamation-triangle"></i> Debe seleccionar una fecha de inicio
							</div>
						</div>

						<div class="col-md-6 mb-2">
							<label for="fechaFinExcel" class="form-label fw-bold">Fecha Fin</label>
							<input type="date" name="fechaFinExcel" id="fechaFinExcel" class="form-control">
							<div class="alert alert-danger" role="alert" id="fechaFinExcelE" style="display: none">
								<i class="bi bi-exclamation-triangle"></i> Debe seleccionar una fecha de fin
							</div>
						</div>
					</div>
				</div>
				<div class="modal-footer d-flex justify-content-center gap-2">
					<button type="button" class="btn btn-primary" id="btnExportarExcel">Exportar</button>
	...
	<script>
		...
		/* document.getElementById('abrirNovedades').addEventListener('click', function(event) {
			//event.preventDefault();
			console.log('abrir modal');
			var modalNovedades = new bootstrap.Modal(document.getElementById('modalNovedades'), {
				backdrop: false
			});
			modalNovedades.show();
		}); */
	...
	<script>
		$(document).ready(function() {
			...
			$('#foto').on('change', function(event) {
				...
				for (let i = 0; i < files.length; i++) {
					...
					const img = $('<img>').attr('src', URL.createObjectURL(renamedFile)).css({ width: '100px', margin: '10px' });
					preview.append(img);
					formData.append('photos[]', renamedFile);
				}

				// Actualizar el campo con todos los nombres, separados por comas
				...
				if (result.isConfirmed) {
					...
					$.ajax({
						...
						error: function(jqXHR, textStatus, errorThrown) {
							Swal.fire({
								icon: "error",
								title: "Error al eliminar el peso",
								timer: 1500,
								showConfirmButton: false
							});
						}
		...
		function buscarItems(id_recepcion, raza, totalCambios) {
				...
				$.ajax({
					...
					success: function(data) {
						...
						if (data.status === 'success') {
							...
							// Desbloquear el botón en caso de advertencia
							$('#btnNuevoCriterio').prop('disabled', false);
							Swal.fire({
								title: 'Advertencia',
								text: data.message,
							...
							// Desbloquear el botón en caso de respuesta no válida
							$('#btnNuevoCriterio').prop('disabled', false);
							Swal.fire({
								title: 'Error',
					...
					error: function(jqXHR, textStatus, errorThrown) {
						// Desbloquear el botón en caso de error de petición
						$('#btnNuevoCriterio').prop('disabled', false);
		...
		$('#btnNuevoCriterioCerdo').click(function() {
			...
			if (validacionesCri == "") {
				// Bloquear el botón
				$('#btnNuevoCriterioCerdo').prop('disabled', true);
				...
				$.ajax({
					...
					success: function(data) {
						...
						if (data.status === 'success') {
							...
							// Desbloquear el botón después del éxito
							$('#btnNuevoCriterioCerdo').prop('disabled', false);
						} else if (data.status === 'warning') {
							// Desbloquear el botón en caso de advertencia
							$('#btnNuevoCriterioCerdo').prop('disabled', false);
							Swal.fire({
		...
		$('#crearProveedor').click(function() {
			...
			$('#canales').val('');
			$('#consecutivog').val('');
			if ("<?php echo $_SESSION['usuario']; ?>" == "ADMINISTRADOR") {
				$('#consecutivog').removeAttr("disabled");
			} else {
				$('#consecutivog').attr("disabled", "true");
			}
			...
			$('#lotep').val('');
			$('#observaciones').val('');
			$("#proveedorCerdo").prop("selectedIndex", 0);
			...
			$("#proveedorCerdoE").css("display", "none");
			$('#id_guia').val('');
		...
		$('#btnNuevoProveedor').click(function() {
			...
			if (validacionesF == "") {
				...
				const datosRecepcion = {
					...
					proveedorCerdo: $('#proveedorCerdo').val()
				};
		...
		function buscarGuia(idRecepcion) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					$("#titulo").text("Editar Guia #" + idRecepcion).addClass("me-5");
					...
					$("#proveedorCerdo").val(data[0].proveedor_cerdo);
					$('#remision').css('border', '');
					$('#destino').css('border', '');
					$('#canales').css('border', '');
					$('#ica').css('border', '');
		...
		function validaciones() {
			...
			if ($("#tipo").val() == "CERDO") {
				if ($("#proveedorCerdo").val() == null || $("#proveedorCerdo").val() == "") {
					$('#proveedorCerdoE').css('display', 'block');
					return 'R';
				}
			}
			...
			if (1 == 1) {
				...
				$.ajax({
					...
					error: function(jqXHR, textStatus, errorThrown) {
						...
						Swal.fire({
							icon: "error",
							title: "Error al editar el criterio",
							timer: 1500,
							showConfirmButton: false
						});
					}
		...
		$('#btnEditarCriterioCerdo').click(function() {
			...
			if (1 == 1) {
				// Bloquear el botón
				$('#btnEditarCriterioCerdo').prop('disabled', true);
				
				infoPesoEditCerdo = {
					"id_recepcion_pesos": $('#id_recepcion_pesos').val(),
					"peso": $('#peso').val(),
					"temperaturap": $('#temperaturap').val(),
					"observacion": $('#observacionPeso').val(),
				};
				...
				$.ajax({
					...
					success: function(data) {
						...
						// Desbloquear el botón después del éxito
						$('#btnEditarCriterioCerdo').prop('disabled', false);
					},
					error: function(jqXHR, textStatus, errorThrown) {
						// Desbloquear el botón en caso de error
						$('#btnEditarCriterioCerdo').prop('disabled', false);
		...
		function mostrarFoto(contenido) {
			...
			const imageHTML = `
				<div style="text-align: center;">
					<img src="controlador/uploads/${contenido}" alt="Foto" style="max-width: 100%; height: auto; border-radius: 8px;" />
				</div>
			`;
		...
		$("#exportarExcel").click(function (e) { 
			...
			$("#proveedorExcel").val("");
			$("#fechaInicioExcel").val("");
			$("#fechaFinExcel").val("");
			$("#proveedorExcelE, #fechaInicioExcelE, #fechaFinExcelE").hide();
		});

		$("#btnExportarExcel").click(function (e) { 
			e.preventDefault();
			let proveedor = $("#proveedorExcel").val();
			let fechaInicio = $("#fechaInicioExcel").val();
			let fechaFin = $("#fechaFinExcel").val();

			$("#proveedorExcel, #fechaInicioExcel, #fechaFinExcel").removeClass("is-invalid");
			$("#proveedorExcelE, #fechaInicioExcelE, #fechaFinExcelE").hide();

			if (proveedor == "") {
				$("#proveedorExcel").addClass("is-invalid");
				$("#proveedorExcel").focus();
				$("#proveedorExcelE").show();
				return;
			}

			if (fechaInicio == "") {
				$("#fechaInicioExcel").addClass("is-invalid");
				$("#fechaInicioExcel").focus();
				$("#fechaInicioExcelE").show();
				return;
			}

			if (fechaFin == "") {
				$("#fechaFinExcel").addClass("is-invalid");
				$("#fechaFinExcel").focus();
				$("#fechaFinExcelE").html("<i class='bi bi-exclamation-triangle'></i> Debe seleccionar una fecha de fin").show();
				return;
			}
			...
			if (new Date(fechaInicio) > new Date(fechaFin)) {
				$("#fechaFinExcel").addClass("is-invalid");
				$("#fechaFinExcel").focus();
				$("#fechaFinExcelE").html("<i class='bi bi-exclamation-triangle'></i> La fecha de fin debe ser posterior a la fecha de inicio").show();
				return;
			}

			var form = document.createElement('form');
            form.method = 'POST';
            form.action = 'controlador/imprimirExcelRecepcion.php';
            form.target = '_blank';
            
            var datosExcelInput = document.createElement('input');
            datosExcelInput.type = 'hidden';
            datosExcelInput.name = 'datosExcel';
            datosExcelInput.value = JSON.stringify({proveedor: proveedor, fechaInicio: fechaInicio, fechaFin: fechaFin});
            form.appendChild(datosExcelInput);
