...
<head>
    <title>Planta de Desposte | Fabricantes</title>
...
<body>
    ...
    <main id="main" class="main">
        ...
        <div class="pagetitle">
            <h1>Administrar Fabricantes de Empaque</h1>
        </div>
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title">
                            <center>
                                <a href="#" class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#modalNuevoProveedor" title="Agregar Nueva Guia" id="crearProveedor">AGREGAR FABRICANTE</a>
                            </center>
        ...
        <div class="modal-dialog modal-dialog modal-dialog-centered modal-xl modal-dialog-scrollable">
            ...
            <div class="modal-content">
                <div class="modal-header text-center">
                    <h6 style="margin-bottom: 0px;font-weight: bold;" class="w-100" id="titulo">AGREGAR FABRICANTE</h6>
...
<script>
    $(document).ready(function() {
        $('#jtable').DataTable({
            ...
            "ajax": {
            url: "tablas/TablaFabricanteEmpaque.php",
            type: "post"
            },
            "language": {
    ...
    $('#crearProveedor').click(function() {
        $('#titulo').text('AGREGAR FABRICANTE');
        $('#nit').val('');
        $('#razon').val('');
        $('#direccion').val('');
        $('#municipio').val('');
    ...
    $('#btnNuevoProveedor').click(function() {
        if (validaciones() == "") {
            ...
            $.ajax({
                ...
                data: {
                    datosFafricanteE: datosProveedorP
                },
                success: function(response) {
                    if (response.status === "success") {
                        ...
                        Swal.fire({
                            title: 'Nuevo Fabricante registrado correctamente',
                            text: '',
                            icon: 'success',
                            timer: 1300,
    ...
    function buscarFabricante(id) {
        $('#id').val(id);
        $('#titulo').text('EDITAR FABRICANTE');
        $('#nit').val('');
        $('#razon').val('');
        $('#direccion').val('');
        $('#municipio').val('');
    ...
    $('#btnEditProveedor').click(function() {
        if (validaciones() == "") {
            ...
            $.ajax({
                ...
                url: 'controlador/controlador.php',
                data: {
                    editFabricanteE: datos
                },
                success: function(data) {
                    $("#modalNuevoProveedor").modal('hide');
                    $('#jtable').DataTable().ajax.reload();
                    Swal.fire({
                        title: 'Fabricante modificado satisfactoriamente',
                        text: '',
                        icon: 'success',
                        timer: 1300,
                        showConfirmButton: false
    ...
    $("#nit").on('change', function() {
        var validarNitFabricanteEmpaque = $("#nit").val();
        if (!$("#nit").val() == "" || !($("#nit").val() == null)) {
            $.ajax({
                type: 'POST',
                url: 'controlador/controlador.php',
                data: {
                    validarNitFabricanteEmpaque
                },
                success: function(data) {
                if (data.length < 5) {
