...
// Obtener datos de la guía
    $sqlGuia = "SELECT * FROM despresado WHERE id IN ($guia)";
...
// Obtener datos de la liquidacion
    $sqlDatos = "SELECT 
        ...
        porcentaje_salmuera
    FROM datos_cf_despresado WHERE guia IN ($guia)";
...
// Obtener Total de unidades
    $sqlUnd = "SELECT cantidadPollo FROM despresado WHERE id = '$guia'";
    $rs_operationUnd = mysqli_query($link, $sqlUnd);
    $rowUnd = mysqli_fetch_assoc($rs_operationUnd);

    $totalDesprese = $rowUnd["cantidadPollo"];
    $totalUnidades = $totalDesprese;
//

// Obtener precios unitarios y total de precio venta
    $sqlPrecioUnitario = "SELECT item, precio_unitario FROM precios_unitarios_cf_despresado WHERE guia = '$guiaUnit' AND item IN ($items_str)";
...
// Datos Despacho
    $sqlIdDespacho = "SELECT id_guia, fechaexp FROM guiaspollo WHERE guiaDesprese IN ($guia) AND tipoGuia = 'DESPRESADO'";
...
$objPHPExcel->getActiveSheet()->setCellValue('B' . $fila, number_format(($totalKilosNetos - $totalKilosNetos) + $totalKilosNetos, 2, ',', ''));
...
header('Content-Disposition: attachment;filename="liquidacionDespresado_'.$guia.'.xlsx"');
