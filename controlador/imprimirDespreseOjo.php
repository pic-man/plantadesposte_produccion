...
while($row3 = mysqli_fetch_row($operation3)){
    ...
    // Lista de códigos válidos para procesar
    $codigos_validos = ["059758", "045401", "042788", "042789", "059756", "059757", "059755", "059762", "059760", "059761", "059759"];
    
    if (in_array($codigo, $codigos_validos)) {
        // Actualizar valores comunes
        $items[$codigo]["cajas"] += $row3[2];
        $items[$codigo]["kg"] += $row3[1];
        $items[$codigo]["cajasBase"] += $row3[3];
        
        // Calcular kgNetos según la condición
        if ($row3[4] == "0") {
            $items[$codigo]["kgNetos"] += $row3[1] - (($row3[3] * 1.8) + ($row3[2] * 2));
        } else {
            $items[$codigo]["kgNetos"] += $row3[1];
        }
    }
}
...
$sql11 = "SELECT tipo, fecha_beneficio FROM lotes_guias_desprese WHERE lote = '$lote' AND guia = '$idGuia'";
...
if ($tipo_pollo == "DESPRESE") {
    ...
    $sql12 = "SELECT lote, SUM(kilos) FROM pesos_pollo WHERE guia = '$idGuia' AND 'ENTERO' GROUP BY lote ORDER BY kilos DESC LIMIT 1";
    $rs_operation12 = mysqli_query($link, $sql12);
    if (mysqli_num_rows($rs_operation12) > 0) {
        $loteEntero = mysqli_fetch_row($rs_operation12)[0];
        $sql13 = "SELECT fecha_beneficio FROM lotes_guias_desprese WHERE lote = '$loteEntero' AND guia = '$idGuia'";
        $rs_operation13 = mysqli_query($link, $sql13);
        $fechaBeneficioEntero = mysqli_fetch_row($rs_operation13)[0];
    } else {
        $fechaBeneficioEntero = "";
    }
} else {
    $fechaBeneficioEntero = $fechaBeneficio;
    $sql14 = "SELECT lote, SUM(kilos) FROM pesos_pollo WHERE guia = '$idGuia' AND 'DESPRESE' GROUP BY lote ORDER BY kilos DESC LIMIT 1";
    $rs_operation14 = mysqli_query($link, $sql14);
    if (mysqli_num_rows($rs_operation14) > 0) {
        $loteDesprese = mysqli_fetch_row($rs_operation14)[0];
        $sql15 = "SELECT fecha_beneficio FROM lotes_guias_desprese WHERE lote = '$loteDesprese' AND guia = '$idGuia'";
        $rs_operation15 = mysqli_query($link, $sql15);
        $fechaBeneficioDesprese = mysqli_fetch_row($rs_operation15)[0];
    } else {
...
$totalPorcentaje = ($kilosNetoMej - $kilosNetoDes) / ($kilosNetoDes > 0 ? $kilosNetoDes : 1) * 100;
$totalPorcentajeP = ($kilosNetoMejP - $kilosNetoDesP) / ($kilosNetoDesP > 0 ? $kilosNetoDesP : 1) * 100;
...
$pdf->Cell(15.875, 5, utf8_decode(""), 0, 0, 'C');
...
$pdf->Cell(18, 5, utf8_decode($tipo), "B", 0, 'C');
$pdf->Cell(15.875, 5, utf8_decode(""), 0, 0, 'C');
$pdf->Cell(8, 5, utf8_decode("Nro: "), 0, 0, '');
$pdf->Cell(8, 5, utf8_decode($row[0]), "B", 0, 'C');
$pdf->Cell(15.875, 5, utf8_decode(""), 0, 0, 'C');
$pdf->Cell(22, 5, utf8_decode("Fecha Proceso: "), 0, 0, '');
$pdf->Cell(23, 5, utf8_decode($row[30]), "B", 0, 'C');

$pdf->Ln(8);
$pdf->SetXY($pdf->GetX() + 21.75, $pdf->GetY());
$pdf->Cell(29, 5, utf8_decode("No Pollos Desprese: "), 0, 0, '');
$pdf->Cell(15, 5, utf8_decode($nroPolloDesprese), "B", 0, 'C');
$pdf->Cell(40.75, 5, "", 0, 0, 'C');
$pdf->Cell(31, 5, utf8_decode("Peso Inicial Desprese: "), 0, 0, '');
$pdf->Cell(17, 5, number_format($pesoInicialDesprese, 2), "B", 0, 'C');
$pdf->Cell(40.75, 5, "", 0, 0, 'C');
$pdf->Cell(37, 5, utf8_decode("Fecha Beneficio Desprese: "), 0, 0, '');
$pdf->Cell(23, 5, utf8_decode($fechaBeneficioDesprese), "B", 0, 'C');
$pdf->Ln(8);
$pdf->SetXY($pdf->GetX() + 21.75, $pdf->GetY());
$pdf->Cell(25, 5, utf8_decode("No Pollos Entero: "), 0, 0, '');
$pdf->Cell(15, 5, utf8_decode($nroPolloEntero), "B", 0, 'C');
$pdf->Cell(44.75, 5, "", 0, 0, 'C');
$pdf->Cell(28, 5, utf8_decode("Peso Inicial Entero: "), 0, 0, '');
$pdf->Cell(16, 5, number_format($pesoInicialEntero, 2), "B", 0, 'C');
$pdf->Cell(44.75, 5, "", 0, 0, 'C');
$pdf->Cell(34, 5, utf8_decode("Fecha Beneficio Entero: "), 0, 0, '');
$pdf->Cell(23, 5, utf8_decode($fechaBeneficioEntero), "B", 0, 'C');

$pdf->Ln(10);
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(21.75, 5, "", 0, 0, 'C');
$pdf->Cell(49, 8, "", 1, 0, 'C');
$pdf->Cell(92.5, 4, utf8_decode("Peso por Desprese"), 1, 0, 'C');
$pdf->Cell(5, 4, "", 0, 0, 'C');
...
if ($items["059762"]["kg"] != 0) {
    ...
    $pdf->Cell(14.5, 10, $nroPolloDesprese, 1, 0, 'C');
...
if ($items["059761"]["kg"] != 0) {
    ...
    $pdf->Cell(14.5, 10, $nroPolloDesprese * 2, 1, 0, 'C');
...
ob_end_clean();
$pdf->Output('formato_desprese.pdf', 'I');
?>