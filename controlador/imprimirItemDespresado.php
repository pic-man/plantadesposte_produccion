...
if ($guia == "" /* || $item == "" || $cantidad == "" || $lote == "" */) {
    header("Location: ../despresado.php");
    exit();
}
...
$sqlItems = "SELECT p.descripcion AS item, i.lote, i.cantidad FROM impresiones_items_despresado i INNER JOIN plantilla p ON i.item = p.item WHERE i.guia = '$guia' AND i.status = 'PENDIENTE'";
//$sqlItems = "SELECT p.descripcion AS item, i.lote, i.cantidad FROM impresiones_items_despresado i INNER JOIN plantilla p ON i.item = p.item WHERE i.guia = '$guia'";
$rs_operationItems = mysqli_query($link, $sqlItems);

if (mysqli_num_rows($rs_operationItems) > 0) {
    $sqlUpdate = "UPDATE impresiones_items_despresado SET status = 'IMPRESO' WHERE guia = '$guia'";
...
$sql3 = "SELECT fecha_beneficio, fecha_vencimiento FROM lotes_guia_despresado WHERE guia = '$guia' ORDER BY id ASC LIMIT 1";
...
$sql4 = "SELECT concentracion FROM despresado WHERE id = '$guia'";
...
while ($row = mysqli_fetch_assoc($rs_operationItems)) {
    for ($i = 0; $i < $row["cantidad"]; $i++) {
        ...
        $pdf->Cell(95, 4, utf8_decode('Instrucciones de uso: Cocinar o Asar'), 0, 1, 'C');
    
        // Ingredientes
        $pdf->SetFont('Arial', 'B', 14);
        $pdf->SetXY($xBorde + 5, $pdf->GetY() + 3);
        $pdf->MultiCell(128.5, 5, utf8_decode($ingredientes), 0, 'C');
    
        // Registro sanitario
        $pdf->SetFont('Arial', 'B', 12);
        $pdf->SetXY($xBorde + 21.75, $pdf->GetY() + 3);
        $pdf->Cell(95, 4.5, utf8_decode('Registro Sanitario: RSA-0033503-2024'), 0, 1, 'C');
...
$pdf->Output('formato_item_'.$guia.'_'.date('YmdHis').'.pdf', 'I'); //D